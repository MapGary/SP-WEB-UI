...
Команда для запуска по умолчанию выполняется в одном браузере (файл запуска `singleBrowser.xml`):
...
Команда для запуска всех тестов в браузерах Chrome, Edge, Yandex:

   ```bash     
       mvn clean test -Dsuite=all       
   ```

Команда для параллельного запуска тестов с указанием файла запуска (пока не совсем разобрался как работает):
